...
public class UserDAO extends DAO implements Authentication{
	...
	public void setRecipe_like(int recipe_id, String user_id, String isLiked) {
		updateTable("INSERT INTO recipe_likes VALUES(?,?,?)", e -> {
			try {
				e.setInt(1, recipe_id);
				e.setString(2, user_id);
				e.setString(3, isLiked);
			} catch (SQLException e1) {
				e1.printStackTrace();
			}
		});
	...
	public void setRecipe_bookmark(int recipe_id, String user_id, String isBookmarked) {
		updateTable("INSERT INTO recipe_likes VALUES(?,?,?)", e -> {
			try {
				e.setInt(1, recipe_id);
				e.setString(2, user_id);
				e.setString(3, isBookmarked);
			} catch (SQLException e1) {
				e1.printStackTrace();
			}
		});
